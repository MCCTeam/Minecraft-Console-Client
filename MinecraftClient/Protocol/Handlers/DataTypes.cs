...
namespace MinecraftClient.Protocol.Handlers
{
    ...
    class DataTypes
    {
        ...
        public Item ReadNextItemSlot(Queue<byte> cache, ItemPalette itemPalette)
        {
            ...
            if (protocolversion > Protocol18Handler.MC113Version)
            {
                ...
                if (itemPresent)
                {
                    ...
                    Console.WriteLine("Got ITEM NBT DATA: ");
                    Console.WriteLine("-----------------------------");

                    foreach (string x2 in GetNbtAsString(nbt, true))
                    {
                    ...
                    Console.WriteLine("-----------------------------");
                    foreach (KeyValuePair<string, object> i in nbt)
                    {
                        if (i.Value is object[])
                        {
                            ConsoleIO.WriteLine(i.Key.ToString() + " = {");

                            foreach (object o in (object[])i.Value)
                            {
                                ConsoleIO.WriteLine("\t{");
                                foreach (KeyValuePair<string, object> o2 in ((Dictionary<string, object>)o))
                                {
                                    ConsoleIO.WriteLine("\t\t" + o2.Key.ToString() + " = " + o2.Value.ToString() + ",");
                                }
                                ConsoleIO.WriteLine("\t},\n");
                            }
                        ...
                        else ConsoleIO.WriteLine(i.Key.ToString() + " = " + i.Value.ToString() + ",");
                    }
                    Console.WriteLine("-----------------------------");
        ...
        public List<string> GetNbtAsString(Dictionary<string, object> nbt, bool root, string depth = "")
        {
            List<string> strings = new List<string>();

            if (nbt == null || nbt.Count == 0)
            {
                strings.Add("TAG_End - No count or null");
                return strings;
            }
            ...
            if (root)
            {
                ...
                strings.Add("TAG_Compound('" + rootName + "'):");
            }
            ...
            foreach (var item in nbt)
            {
                ...
                int fieldType;
                string fieldContent = GetNbtFieldAsAString(item.Value, out fieldType, depth);
                strings.Add(GetNbtFieldTypeString(fieldType) + "('" + item.Key + "'):");
                strings.Add(depth + (fieldType == 10 ? "\t" : "") + fieldContent);
            }

            strings.Add("TAG_End"); // TAG_End
            return strings;
        }

        private string GetNbtFieldAsAString(object obj, out int fieldType, string depth)
        {
            if (obj is byte)
            {
                fieldType = 1; // TAG_Byte
                return ByteArrayToString(new byte[] { (byte)obj });
            }
            ...
            else if (obj is object[])
            {
                ...
                string firstItemData = GetNbtFieldAsAString(list[0], out firstItemType, depth);
                list.RemoveAt(0);
                Console.WriteLine("Parsing a TAG_List it has a size of: " + arrayLengthTotal + " and type of: " + GetNbtFieldTypeString(firstItemType));

                // Encode further list items, check they have the same type
                int subsequentItemType;
                List<string> subsequentItems = new List<string>();
                foreach (object item in list)
                {
                    string _item = GetNbtFieldAsAString(item, out subsequentItemType, depth);
                    Console.WriteLine("Adding item to a list: " + _item);
                    subsequentItems.Add(_item);

                    if (subsequentItemType != firstItemType)
                        throw new System.IO.InvalidDataException(
                            "GetNbt: Cannot encode object[] list with mixed types: " + firstItemTypeString + ", " + item.GetType().Name + " into NBT string!");
                }

                string itemsString = "" + GetNbtFieldTypeString(firstItemType) + "('" + firstItemData.Replace("\n", "").Trim() + "'): ";

                foreach (string listStr in subsequentItems)
                {
                    itemsString += listStr;
                }
            ...
            else if (obj is Dictionary<string, object>)
            {
                ...
                depth += "\t";
                List<string> stringList = GetNbtAsString((Dictionary<string, object>)obj, false, depth);

                string itemsString = "";

                foreach (string listStr in stringList)
                {
                    itemsString = depth + listStr;
                    itemsString += "\n";
                }
            ...
            else if (obj is long[])
            {
                ...
                string longListString = "";

                foreach (int item in srcLongList)
                {
                    longListString += item + ",";
                }

                return longListString;
            }
            else
            {
                throw new System.IO.InvalidDataException("GetNbt: Cannot encode data type " + obj.GetType().Name + " into NBT string!");
            }
        }

        private string GetNbtFieldTypeString(int fieldType)
        {
            if (fieldType == 0)
                return "TAG_End";
            ...
            throw new System.IO.InvalidDataException("Invalid Field Type provided: " + fieldType);
        }
